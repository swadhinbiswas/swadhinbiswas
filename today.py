...
def simple_request(func_name, query, variables):
    ...
    for attempt in range(MAX_RETRIES):
        ...
        if request.status_code in [500, 502, 503, 504]:
            print(f"{func_name} failed with {request.status_code}. Retrying in {2**attempt} seconds...")
...
def recursive_loc(owner, repo_name, data, cache_comment, addition_total=0, deletion_total=0, my_commits=0, cursor=None):
    ...
    for attempt in range(MAX_RETRIES):
        ...
        if request.status_code == 200:
            break
        if request.status_code in [500, 502, 503, 504]:
             print(f"recursive_loc failed with {request.status_code}. Retrying in {2**attempt} seconds...")
             time.sleep(2**attempt)
             continue
        break
    if request.status_code == 200:
...
def cache_builder(edges, comment_size, force_cache, loc_add=0, loc_del=0):
    ...
    edges = [e for e in edges if e is not None and e.get('node') is not None and e['node'].get('nameWithOwner') is not None]
...
def flush_cache(edges, filename, comment_size):
    ...
    with open(filename, 'w') as f:
        ...
        for node in edges:
            ...
            if node is None or node.get('node') is None or node['node'].get('nameWithOwner') is None:
                continue
